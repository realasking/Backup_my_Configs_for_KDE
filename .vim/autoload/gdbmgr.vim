...
"    call Decho("clear Assemblys buffer and make it editable")
    setlocal nobuflisted ma noro bh=hide bt=nofile
    sil keepj %d
    let lzkeep= &lz
    set lz

    " display disassmbled code
"    call Decho("install thread information into Assembly buffer")
    call setline(1,mesglist)
    sil! g/^[* ]/j
    keepj $

	" have current line be atop the pc
    call search(curaddr,'cw')
    redraw
    let &lz= lzkeep
...
"    call Decho("mesg<".mesg.">")
	...
	if mesg =~ '\<del\>'
     call setline(iline,printf("#%-3d %15s %15s:%d",brknum,"tmp-".brktype,simplify(brkfile),brkline))
	else
     call setline(iline,printf("#%-3d %15s %15s:%d",brknum,brktype,simplify(brkfile),brkline))
	endif
...
"	call Decho("(CmdPoll while-loop) terminate program-running mode")
	let s:gdbmgr{t:gdbmgrtab}_running = 0
	let &ut                           = s:keeput
	if mode() == "i"
...
"   "   call Dret("s:SourceEdit : s:GdbMgrOpenCodedBuf(S) yielded 0!")
  ...
  call s:gdbmgr_registry_{t:gdbmgrtab}["M"].Update("note: editing a source file invalidates the binary!")
...
"  call Decho("srcbuf<".srcbuf."> srcwin#".srcwin." curwin#".winnr()." expand(%)<".expand("%").">")
  ...
  if srcwin == -1
   " the current source buffer is not showing.  Presumably we now have a new source buffer
...
"	call Decho("buffer appears previously to have been a source buffer")
  ...
  " even though this source-code buffer may have been seen before and kept as hidden, it still
  " needs to have its signs updated.
...
"   call Decho('doing "W" menu')
  ...
  exe 'sil! menu 500.410 '.g:DrChipTopLvlMenu.'Gdbmgr.Go\ to\ Assembly	:call gdbmgr#GdbMgrForeground("A")<cr>'
  exe 'sil! menu 500.410 '.g:DrChipTopLvlMenu.'Gdbmgr.Go\ to\ Breakpoints	:call gdbmgr#GdbMgrForeground("B")<cr>'
  exe 'sil! menu 500.420 '.g:DrChipTopLvlMenu.'Gdbmgr.Go\ to\ Commands	:call gdbmgr#GdbMgrForeground("C")<cr>'
  exe 'sil! menu 500.430 '.g:DrChipTopLvlMenu.'Gdbmgr.Go\ to\ Expressions	:call gdbmgr#GdbMgrForeground("E")<cr>'
  exe 'sil! menu 500.440 '.g:DrChipTopLvlMenu.'Gdbmgr.Go\ to\ Funcstack	:call gdbmgr#GdbMgrForeground("F")<cr>'
  exe 'sil! menu 500.450 '.g:DrChipTopLvlMenu.'Gdbmgr.Go\ to\ Messages	:call gdbmgr#GdbMgrForeground("M")<cr>'
  exe 'sil! menu 500.460 '.g:DrChipTopLvlMenu.'Gdbmgr.Go\ to\ Source	:call gdbmgr#GdbMgrForeground("S")<cr>'
  exe 'sil! menu 500.470 '.g:DrChipTopLvlMenu.'Gdbmgr.Go\ to\ Watchpoints	:call gdbmgr#GdbMgrForeground("W")<cr>'
...
"  call Dfunc("s:GdbMgrOpenCodedBuf(code<".a:code.">) s:codedbuflist".(exists("s:codedbuflist")? string(s:codedbuflist) : '<n/a>'))
  ...
  " push information onto codedbuf list:
  "    0     1                            2               3
...
"	call Dret("s:GdbMgrOpenCodedBuf ".codewin." : s:codebuflist".string(s:codedbuflist))
	return codewin
   else
...
"	call Dret("s:GdbMgrOpenCodedBuf ".-codebuf." : s:codebuflist".string(s:codedbuflist))
	return -codebuf
   endif
...
"   call Dret("s:GdbMgrOpenCodedBuf 0 : s:codebuflist".(exists("s:codedbuflist")? string(s:codedbuflist) : '<n/a>'))
   return 0
  endif
...
"   call Decho("quitting temporary one-line window")
   sil! q
  endif

  " return to the original window
  let origwin= codebufentry[2]
"  call Decho("returning to original window#".origwin)
...
"  call Dret("s:GdbMgrCloseCodedBuf : s:codebuflist".(exists("s:codebuflist")? string(s:codebuflist) : '<n/a>'))
endfun
...
"   call Decho("initializing GdbMgr: libcall'ing gdbmgr.so<".a:tgtfunc.">")
  ...
  " if tgtfunc is gmGdb, then the first byte is S==stopped R=running Q=query
...
"  call Decho("code   <".code."> buf#".bufnr("%"))
...
"  call Decho('(BufSwitch) setting <c-f6> to call gdbmgr#BufSwitch() in buf#%'.bufnr("%"))
  nn <buffer> <silent> <c-F6>	:call gdbmgr#BufSwitch()<cr>
"  call Dret("gdbmgr#BufSwitch : showing code<".nxtcode.">'s buf#".bufnr("%"))
endfun
...
"	call Decho("winspec<".string(winspec)."> type#".type(winspec))
    if type(winspec) == 3
...
"	call Decho("determine wildrows: rowspec<".rowspec."> fixedrows=".fixedrows)
   else
	let wildcnt   = wildcnt + 1
	let fixedrows = fixedrows + 1
"	call Decho("determine wildrows: wildcnt=".wildcnt." fixedrows=".fixedrows)
   endif
  endfor
...
"   call Decho("hsplits: rowspec<".rowspec.">")
  ...
  for rowspec in a:winctrl
   ...
   for colspec in colspecs
    ...
    if codes =~ 'S'
	 ...
	 sil! enew
	 file [Source]
	 setlocal nobuflisted ro nomod bt=nofile
     let t:srcbuf= bufnr("%")
...
"	 call Decho("set s:gdbmgr_registry_".t:gdbmgrtab."[".bufnr("%")."].code= ".code)
	 let i= i - 1
	endwhile
"	call Decho("--- codes<".codes."> now registered ---")
    sil wincmd l
